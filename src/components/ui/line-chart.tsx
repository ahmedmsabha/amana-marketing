...
export function LineChart({ 
  ...
  const { maxValue, minValue, points, pathData } = useMemo(() => {
    ...
    const calculatedPoints: ChartPoint[] = data.map((item, index) => {
      ...
      const y = chartHeight - ((item.value - adjustedMin) / (adjustedMax - adjustedMin)) * chartHeight;
  ...
  return (
    <div className={`bg-gray-800 rounded-lg p-6 border border-gray-700 ${className}`}>
      ...
      <div className="relative" style={{ height: `${height}px` }}>
        ...
        {/* Y-axis labels */}
        <div className="absolute top-0 left-0 bottom-0 flex flex-col justify-between py-10 -ml-12">
